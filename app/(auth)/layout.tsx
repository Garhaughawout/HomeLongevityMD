      ...
      <div className="grid w-full gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <section className="space-y-5">
          ...
          <h1 className="text-4xl font-semibold leading-tight text-balance sm:text-5xl">
            Secure staff access for client operations and assessment workflows.
          </h1>
          <p className="max-w-xl text-base leading-8 text-[color:var(--muted)]">
            Sign in with your Supabase-authenticated account to access the
            internal dashboard, client records, assessments, quotes, and
            activity history.
          </p>
        </section>
        <div className="surface rounded-[2rem] p-6 sm:p-8">{children}</div>
      </div>
    </div>
  );
}