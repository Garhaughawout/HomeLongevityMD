...
async function getAllAssessments(): Promise<AssessmentWithClient[]> {
  ...
  return assessments.map((a) => ({ ...a, client_name: nameMap.get(a.client_id) ?? "Unknown" }));
}
...
export default async function AssessmentsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-[color:var(--foreground)]">Assessments</h1>
      ...
      {assessments.length === 0 ? (
        <div className="rounded-2xl p-8 text-center" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          <p className="text-sm" style={{ color: "var(--muted)" }}>No assessments yet. Complete a client intake to generate one.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Risk</th>
                <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Score</th>
                <th className="px-5 py-3 text-right text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Date</th>
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: "var(--border)" }}>
              {assessments.map((a) => {
                const style = CATEGORY_STYLES[a.risk_category] ?? CATEGORY_STYLES.moderate;
                return (
                  <tr key={a.id} className="hover:bg-black/[0.02]">
                    <td className="px-5 py-3">
                      <Link href={`/clients/${a.client_id}/assessments`} className="font-medium hover:underline" style={{ color: "var(--foreground)" }}>
                        {a.client_name}
                      </Link>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3 text-right tabular-nums font-semibold" style={{ color: "var(--ink)" }}>
                      {a.aggregate_score}<span className="text-xs font-normal" style={{ color: "var(--muted)" }}>/100</span>
                    </td>
