...
function AssessmentCard({
	...
	assessment: RiskAssessmentRow;
	clientId: string;
}) {
	const style =
		CATEGORY_STYLES[assessment.risk_category] ?? CATEGORY_STYLES.moderate;
	return (
		<div
		...
		>
			<div className="flex items-start justify-between gap-4">
				...
				<div className="flex flex-col items-end gap-2">
					...
					<Link
						href={`/clients/${clientId}/quotes?assessment_id=${assessment.id}`}
			...
			<div className="grid gap-3 sm:grid-cols-2">
				<ScoreBar
					score={assessment.home_safety_score}
					label="Home Safety"
				/>
				<ScoreBar score={assessment.mobility_score} label="Mobility" />
				<ScoreBar
					score={assessment.adls_iadls_score}
					label="ADLs / IADLs"
				/>
				<ScoreBar
					score={assessment.cognition_score}
					label="Cognition"
				/>
				<ScoreBar
					score={assessment.fall_risk_score}
					label="Fall Risk"
				/>
				<ScoreBar
					score={assessment.caregiver_support_score}
		...
		return (
			...
			>
				...
				<p className="mt-1 text-sm" style={{ color: "var(--muted)" }}>
					Complete the intake first — assessments are generated from
					submitted intake data.
				</p>
				<Link
					href={`/clients/${client.id}/intake`}
					className="mt-4 inline-block px-4 py-2 rounded-lg text-sm font-medium text-white"
