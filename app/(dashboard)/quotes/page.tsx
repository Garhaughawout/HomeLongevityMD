...
async function getAllQuotes(): Promise<
	(QuoteRow & { client_name?: string })[]
> {
	...
	return quotes.map((q) => ({
		...q,
		client_name: nameMap.get(q.client_id) ?? "Unknown",
	}));
}
			...
			{quotes.length === 0 ? (
				...
				>
					<p className="text-sm" style={{ color: "var(--muted)" }}>
						No quotes yet. Open a client and create one from the
						Quotes tab.
					</p>
				</div>
			) : (
