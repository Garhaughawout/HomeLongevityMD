...
function scoreAdlsIadls(d: AdlsIadlsData | null | undefined): number {
  ...
  const iadlKeys = [
    "meal_preparation", "medication_management", "housekeeping", "laundry",
    "transportation", "shopping", "finances", "telephone_use",
  ] as const;
...
function scoreFallRisk(d: FallRiskData | null | undefined): number {
  ...
  if (d.berg_balance_score !== undefined && d.berg_balance_score < 45) pts += 15;

  return clamp(pts);
}

function scoreCaregiverSupport(d: CaregiverSupportData | null | undefined): number {
  if (!d || Object.keys(d).length === 0) return 50;
  let pts = 0;

  if (d.has_informal_caregiver === false) pts += 30;
  if (!d.caregiver_lives_with_client && !d.daily_check_in_available) pts += 15;
  ...
  if (d.caregiver_hours_per_week !== undefined && d.caregiver_hours_per_week < 7) pts += 10;
...
function deriveCategory(
  ...
  if (physicianOverride) {
    ...
    const override = CATEGORY_ORDER.indexOf(physicianOverride as RiskCategory);
...
export function scoreIntake(intake: ClientIntakeRow): ScoringResult {
  ...
  const cs  = scoreCaregiverSupport(intake.caregiver_support as CaregiverSupportData | null);
