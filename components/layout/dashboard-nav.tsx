...
function HomeIcon() {
  return (
    <svg fill="none" viewBox="0 0 24 24" strokeWidth={1.75} stroke="currentColor" className="h-full w-full">
...
export function DashboardNav({ orientation = "vertical" }: DashboardNavProps) {
  ...
  return (
    <nav className="flex flex-col gap-0.5">
      ...
      {(() => {
        const active = isActive(pathname, secondaryNav.href, secondaryNav.exact);
        return (
          <Link
          ...
          >
            <span className="h-5 w-5 shrink-0">{secondaryNav.icon}</span>
            {secondaryNav.label}
          </Link>
        );
      })()}
