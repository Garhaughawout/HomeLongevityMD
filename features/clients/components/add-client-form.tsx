...
function Field({ label, name, type = "text", placeholder, required, errors, hint }: FieldProps) {
  const id = `field-${name}`;
  const hasError = errors && errors.length > 0;
  return (
    <div>
      <label htmlFor={id} className="mb-1 block text-sm font-medium text-[color:var(--foreground)]">
        {label}
      ...
      />
      {hint && !hasError && <p className="mt-1 text-xs text-[color:var(--muted)]">{hint}</p>}
      {hasError && <p className="mt-1 text-xs text-rose-600">{errors[0]}</p>}
    </div>
  );
}
      ...
      <fieldset className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="sm:col-span-1">
            <Field label="City" name="city" placeholder="Springfield" errors={state.errors?.city} />
          </div>
          <Field
            label="State"
            name="state"
          ...
          />
          <Field label="ZIP" name="zip" placeholder="62701" errors={state.errors?.zip} />
        </div>
      </fieldset>
