...
import { services, financialComparison } from "@/features/marketing/landing/content";
...
export function LandingPage() {
  return (
    <SiteFrame>
      ...
      <section className="grid gap-12 py-6 lg:grid-cols-2 lg:items-center lg:py-14">
        <div className="space-y-6">
          ...
          <h1 className="text-5xl font-semibold leading-tight text-balance sm:text-6xl">
            Clarity Today.{" "}
            <span className="text-[color:var(--navy)]">Independence Tomorrow.</span>
          </h1>
          <p className="text-lg leading-8 text-[color:var(--muted-strong)]">
            Physician-led functional home assessments helping older adults safely
            age in place — and giving families the medically-guided answers they
            need before a crisis forces the decision.
          </p>
        ...
        <div className="navy-panel rounded-xl p-8 space-y-4">
          ...
          <p className="text-sm text-white/70 leading-7">
            Board-certified physical medicine and rehabilitation specialist with
            over a decade of clinical experience at Nebraska Spine + Pain Center
            in Omaha. Dr. Haughawout brings deep expertise in diagnostics,
            functional medicine, and patient-centered care to HomeLongevityMD —
            applying that clinical foundation to help older adults and their
            families plan for safer, more independent lives at home.
          </p>
          <div className="border-t border-white/10 pt-4">
            <p className="text-xs text-white/50">HomeLongevityMD.com &nbsp;&middot;&nbsp; Omaha, NE</p>
          </div>
        </div>
      </section>
      ...
      <section id="why-us" className="navy-panel rounded-xl overflow-hidden">
        <div className="grid gap-0 lg:grid-cols-[1fr_auto]">
          <div className="p-8 lg:p-12 space-y-5">
            ...
            <p className="text-sm leading-7 text-white/72 max-w-sm">
              A fall, a hospitalization, a sudden decline — these events leave families
              scrambling for answers under pressure. A professional evaluation now helps
              avoid reactive, costly transitions and plans for a safer future on your terms.
            </p>
          </div>
          <div className="flex flex-col divide-y divide-white/10 border-t border-white/10 lg:border-l lg:border-t-0 lg:divide-y lg:divide-white/10">
            {financialComparison.map((item) => (
              <div key={item.label} className="px-10 py-7 space-y-1">
      ...
      <section id="our-difference" className="space-y-10">
        ...
        <div className="grid gap-0 divide-y border-t border-b border-[color:var(--border)] divide-[color:var(--border)] md:grid-cols-3 md:divide-x md:divide-y-0">
          {(
            ...
            <div key={item.label} className="px-0 py-8 md:px-8 md:first:pl-0 md:last:pr-0 space-y-3">
              <p className="text-3xl font-bold text-[color:var(--accent)]">{item.num}</p>
              <h3 className="text-base font-semibold">{item.label}</h3>
              <p className="text-sm leading-7 text-[color:var(--muted-strong)]">{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="contact" className="navy-panel rounded-xl px-8 py-14 lg:px-14">
        <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
          <div className="space-y-4">
            ...
            <p className="text-base text-white/70 leading-7">
              Reach out directly — we serve patients and families across the Omaha, Nebraska area.
            </p>
          </div>
          <div className="flex flex-col gap-6 lg:pl-10 lg:border-l lg:border-white/10">
            <div className="space-y-1">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">Phone</p>
              <p className="text-2xl font-semibold text-white">(402) 555-0000</p>
            </div>
            <div className="space-y-1">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-white/40">Email</p>
              <p className="text-lg font-medium text-white">info@homelongevitymd.com</p>
            </div>
          </div>
        </div>
      </section>
