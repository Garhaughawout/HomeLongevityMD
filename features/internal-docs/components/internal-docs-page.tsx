  ...
  return (
    <section className="space-y-4">
      ...
      <div className="border-y border-black">
        {items.map((item) => (
          <article key={item.path} className="border-b border-black/80 py-5 last:border-b-0">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="space-y-1.5">
                <p className="font-mono text-sm font-semibold text-slate-950">{item.path}</p>
            ...
            <p className="mt-3 text-sm leading-6 text-slate-800">{item.notes}</p>
          </article>
        ))}
      </div>
    </section>
...
function ApiReferenceTable({ title, description, items }: ApiReferenceTableProps) {
  return (
    <section className="space-y-4">
      <div className="space-y-1">
        <h2 className="text-2xl font-semibold text-slate-950">{title}</h2>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="border-t border-slate-600 bg-white px-5 py-5">
              <p className="mb-3 text-sm leading-6 text-slate-700 sm:hidden">{item.purpose}</p>
              <p className="text-sm leading-6 text-slate-800">{item.notes}</p>
              ...
              <div className="mt-5">
                ...
                <div className="overflow-hidden rounded border border-slate-600">
                  {item.responseExamples.map((example, idx) => (
                    <details
                      key={example.label}
                    ...
                    >
                      <summary className="flex cursor-pointer list-none items-start gap-3 bg-white px-4 py-3 hover:bg-slate-50 [&::-webkit-details-marker]:hidden">
                        ...
                        >
                          {typeof example.statusCode === "number" ? example.statusCode : "var"}
                        </span>
    ...
    <div className="bg-white">
      <section className="px-6 py-10 lg:px-8 lg:py-12">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-700">
          This page documents the routes and endpoints currently implemented in
          the application. It is protected behind the dashboard auth boundary so
          the reference stays internal to the team.
        </p>
      </section>
