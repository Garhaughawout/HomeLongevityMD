...
export const pageReference: ReadonlyArray<InternalDocEntry> = [
  {
    path: "/",
    access: "public",
    purpose: "Public landing page for the product foundation and roadmap positioning.",
  ...
  },
  {
    ...
    purpose: "Protected dashboard home. Will show KPI cards, high-risk alerts, and recent activity in Phase 9.",
    notes: "Unauthenticated visitors are redirected to /login by middleware.",
  },
  {
    path: "/clients",
    access: "authenticated",
    purpose: "Searchable client list with name search, status filter, sort, and pagination. Includes add-client dialog.",
  ...
  },
  {
    path: "/clients/[id]",
    access: "authenticated",
    purpose: "Client detail workspace — Overview tab with profile and address cards.",
  ...
  },
  {
    ...
    access: "authenticated",
    purpose: "Client assessments tab — placeholder shell; risk scoring arrives in Phase 6.",
    notes: "Will list persisted risk_assessments rows keyed to this client.",
  },
  {
    path: "/clients/[id]/quotes",
    access: "authenticated",
    purpose: "Client quotes tab — placeholder shell; quote generation arrives in Phase 7.",
    notes: "Will list quotes rows with status, rate, and revision history.",
  },
  {
    path: "/clients/[id]/notes",
    access: "authenticated",
    purpose: "Client notes tab — placeholder shell; notes CRUD arrives in Phase 8.",
    notes: "Soft-deleted notes stored in the notes table with is_deleted flag.",
  },
  {
    path: "/clients/[id]/activity",
    access: "authenticated",
    purpose: "Client activity tab — placeholder shell; event logging arrives in Phase 8.",
    notes: "Will render activity_log rows scoped to this client_id, newest first.",
  },
  {
    path: "/assessments",
    access: "authenticated",
    purpose: "Assessment overview listing all structured intake and risk scoring records.",
    notes: "Shell in place. Multi-step intake wizard and risk scoring arrive in Phases 5 and 6.",
  },
  {
    path: "/quotes",
    access: "authenticated",
    purpose: "Quote management listing generated pricing, service recommendations, and revision history.",
    notes: "Shell in place. Pricing utilities and quote generation arrive in Phase 7.",
  },
  {
    path: "/activity",
    access: "authenticated",
    purpose: "Audit trail for all internal actions including notes, status changes, and workflow events.",
    notes: "Shell in place. Automatic activity logging across all mutations arrives in Phase 8.",
  },
  {
    path: "/dashboard/docs",
    access: "authenticated",
    purpose: "Internal-only reference page for routes, APIs, and implementation surfaces.",
...
export const apiReference: ReadonlyArray<ApiReferenceEntry> = [
  {
    ...
    access: "internal-api",
    method: "GET",
    purpose: "Server-side connectivity check that verifies the configured Supabase project is reachable.",
    notes: "Returns safe status information only and does not expose credentials or table data.",
    responseExamples: [
      {
        label: "Success",
        statusCode: 200,
        description: "Supabase project is reachable and accepted the auth settings health check.",
        body: `{
  "ok": true,
  ...
  "projectHost": "lyouvaplrfcozlylauge.supabase.co"
      ...
      },
      {
        ...
        statusCode: 500,
        description: "The application is missing required Supabase environment variables.",
        body: `{
  "ok": false,
  "message": "Supabase environment variables are missing."
      ...
      },
      {
        ...
        statusCode: 503,
        description: "The runtime could not reach the configured Supabase project over the network.",
        body: `{
  "ok": false,
  "message": "Unable to reach the Supabase project from the application runtime."
      ...
      },
      {
        ...
        description: "Supabase responded but rejected the request, for example with an invalid key or project configuration issue. The route returns the upstream HTTP status.",
        body: `{
  "ok": false,
